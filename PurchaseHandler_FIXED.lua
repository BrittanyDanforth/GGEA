--[[
	✨ ULTRA POLISHED Purchase Handler v2.1 - FIXED DROP COLLECTION
...
local CONFIG = {
	...
	-- CRITICAL: Drop Collection Coordination Settings
	DROP_COLLECTION_DELAY = 0.5,  -- Wait time before collecting drops
	DROP_PROCESSING_TIME = 1.0,    -- Time to allow DropperCore to process
	MODEL_DROP_CHECK_INTERVAL = 0.1, -- How often to check for model drops
}
...
-- CRITICAL: Track drops being processed by DropperCore
...
-- CRITICAL FIX: Enhanced auto-collect that waits for drops to be processed
...
local function setupAutoCollect(player)
	...
	-- CRITICAL FIX: Enhanced auto-collect connection with drop coordination
	autoCollectConnections[player] = Money.Changed:Connect(function(newValue)
		...
		if newValue > 0 and autoCollectEnabled[player] ~= false then
			-- CRITICAL: Wait longer to allow DropperCore to process drops
			task.wait(CONFIG.DROP_COLLECTION_DELAY)
			
			-- Double-check that money is still there (not collected by manual collection)
...
-- ========================================
-- CRITICAL FIX: PART COLLECTOR FOR MODEL DROPS
-- ========================================
for _, collector in ipairs(essentials:GetChildren()) do
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			-- CRITICAL: Check if this part belongs to a Model with Cash
				...
				for _, descendant in ipairs(model:GetDescendants()) do
					if descendant:IsA("BasePart") and descendant:FindFirstChild("Cash") then
						...
						if cash and cash:IsA("IntValue") then
							...
							print("💰 [PART COLLECTOR] Found Cash in", model.Name, "part:", descendant.Name, "= $" .. cash.Value)
						end
					end
				end
			end
			
			-- Handle model drops differently
			if isModelDrop and modelCashValue > 0 then
				...
				print("✅ [PART COLLECTOR] Collecting model drop:", model.Name, "with $" .. modelCashValue)
			...
			-- Original logic for simple parts with Cash
...
-- ========================================
-- MONEY COLLECTOR WITH PER-PLAYER STEALING
-- ========================================
...
-- ========================================
print("✅ ULTRA POLISHED Purchase Handler v2.1 - FIXED DROP COLLECTION loaded!")
...
print("⏱️ Drop Collection Delay:", CONFIG.DROP_COLLECTION_DELAY, "seconds")
print("🔄 Drop Processing Time:", CONFIG.DROP_PROCESSING_TIME, "seconds")