--[[
	✨ Cinnamoroll Purchase Handler - CONFLICT FIXED
	
	🔧 FIX: Removed ProcessReceipt handler (MoneyShop handles cash now!)
	
	This script now ONLY handles:
	✅ Gamepass purchases
	✅ Auto-Collect toggle
	✅ 2x Cash visual indicators
	✅ Button interactions
	
	Developer Products (cash) are handled by MoneyShop.server.lua!
--]]
...
print("🔍 [Cinnamoroll] ========== FINDING TYCOON ID ==========")
print("🔍 [Cinnamoroll] Script location:", script:GetFullName())
print("🔍 [Cinnamoroll] script.Parent:", script.Parent.Name, "(" .. script.Parent.ClassName .. ")")
print("🔍 [Cinnamoroll] script.Parent path:", script.Parent:GetFullName())

local TYCOON_ID = script.Parent:GetAttribute("TycoonId") or script.Parent.Name
print("🔍 [Cinnamoroll] Selected TYCOON_ID:", TYCOON_ID)
...
if not datastoreSuccess then
	warn("[Cinnamoroll] DataStore unavailable:", datastoreError)
end
...
local function saveAutoCollectPreference(player, enabled)
	...
	if not success then
		warn("[Cinnamoroll] Failed to save auto-collect preference:", err)
	end
...
local function resetTycoonPurchases()
	print("🔄 [Cinnamoroll] RESETTING PURCHASE HANDLER (MULTI-TYCOON SAFE MODE)...")
	...
	if partStorage then
		...
		print("  ✓ [Cinnamoroll] Cleared PartStorage:", storageCount, "drops")
	end
	...
	if CONFIG.USE_ATTRIBUTE_CLEANUP then
		...
		print("  ✓ [Cinnamoroll] Destroyed", destroyedCount, "owned drops (attribute-based)")
	...
	elseif CONFIG.USE_BOUNDING_BOX_FALLBACK then
		...
		print("  ✓ [Cinnamoroll] Destroyed", destroyedCount, "drops (bounding box fallback)")
	end
...
Players.PlayerRemoving:Connect(function(player)
	if tycoonOwner.Value == player then
		print("👋 [Cinnamoroll] PLAYER REMOVING:", player.Name, "- FORCING CLEANUP")
...
tycoonOwner.Changed:Connect(function()
	...
	if newOwner == nil and currentOwner ~= nil then
		...
		print("👋 [Cinnamoroll] Owner left (via Owner.Changed), resetting purchases...")
		resetTycoonPurchases()
		currentOwner = nil
...
MarketplaceService.PromptGamePassPurchaseFinished:Connect(function(player, gamePassId, wasPurchased)
	...
	print("🛍️ [Cinnamoroll] Gamepass purchased:", gamePassId, "by", player.Name)
...
print("✅ [Cinnamoroll] Purchase Handler CONFLICT-FREE VERSION loaded!")
print("🛡️ [Cinnamoroll] Multi-Tycoon Safe: ENABLED")
...
print("💰 [Cinnamoroll] Developer Products: Handled by MoneyShop.server.lua")
