--[[
	✨ HelloKitty Purchase Handler - PRODUCTION VERSION
...
local function createMinimalParticles(position)
	...
	emitter.SpreadAngle, emitter.Color, emitter.Size = Vector2.new(15, 15), ColorSequence.new(Color3.new(1, 0.8, 0.9)), NumberSequence.new(0.3)
...
local function update2xCashIndicator(giver, has2xCash)
	...
	if has2xCash and not indicator2x then
		...
		local f = Instance.new("Frame") f.Size, f.BackgroundColor3, f.BackgroundTransparency, f.BorderSizePixel, f.Parent = UDim2.new(1, 0, 1, 0), Color3.fromRGB(255, 215, 0), 0.2, 0, bb
...
local function setupAutoCollect(player)
	...
	if giver then
		...
		if autoCollectEnabled[player] ~= false then
			f.BackgroundColor3, f.BackgroundTransparency, l.Text, l.TextColor3 = Color3.new(0, 0.7, 0.7), 0.2, "AUTO", Color3.new(1, 1, 1)
		else
...
autoCollectToggle.OnServerEvent:Connect(function(player, enabled)
	...
	if giver then
		...
		if indicator then
			...
			if label then
				if enabled then
					label.Text, label.TextColor3, frame.BackgroundColor3 = "AUTO", Color3.new(1, 1, 1), Color3.new(0, 0.7, 0.7)
				else
...
local function resetTycoonPurchases()
	...
	if partStorage then
		...
		print("  ✓ [HelloKitty] Cleared PartStorage:", storageCount)
	end
	if CONFIG.USE_ATTRIBUTE_CLEANUP then
		...
		print("  ✓ [HelloKitty] Destroyed", destroyedCount, "owned drops")
	elseif CONFIG.USE_BOUNDING_BOX_FALLBACK then
		...
		print("  ✓ [HelloKitty] Destroyed", destroyedCount, "drops")
	end
	Money.Value = 0
...
for _, collector in ipairs(essentials:GetChildren()) do
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			...
			if model and model:IsA("Model") and (model.Name:match("^Drop_") or model.Name:match("^HelloKitty") or model.Name:match("^HK_") or model.Name:match("^Sanrio") or model.Name:match("^Kitty")) then
...
print("✅ [HelloKitty] PRODUCTION VERSION loaded!")
