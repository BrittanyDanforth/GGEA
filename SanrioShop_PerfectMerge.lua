...
local CARD_WIDTH = 0.1    -- % of grid cell width the card uses (smaller = less crop)
...
local function getGamePassInfo(id)
    local key = "pass_" .. id
    local c = productCache:get(key); if c then return c end
...
local function confetti(parent)
    for i = 1, 8 do
        ...
        chip.BackgroundColor3 = (i % 2 == 0) and theme.accent or theme.kuromi
        ...
        task.spawn(function()
            TweenService:Create(chip, TweenInfo.new(0.45), {
                Position = UDim2.new(chip.Position.X.Scale, (math.random() - 0.5) * 60, 0, math.random(60, 110)),
...
function Shop:createProductItem(product, productType, parent)
    ...
    -- Visible card background (sized by CARD_WIDTH/CARD_ASPECT and centered)
    local bg = Instance.new("ImageLabel")
    bg.Name = "CardBG"
    bg.AnchorPoint = Vector2.new(0.5, 0.5)
    bg.Position = UDim2.fromScale(0.5, 0.5)
    bg.Size = UDim2.new(CARD_WIDTH, 0, CARD_WIDTH / CARD_ASPECT, 0) -- wired to constants
    bg.BackgroundTransparency = 1
    bg.Image = CARD_BG_IMAGE
