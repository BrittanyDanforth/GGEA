...
function Signal.new(): TSignal
	local self = setmetatable({
		_be = Instance.new("BindableEvent"),
		_conns = {}
	}, Signal) :: SignalImpl
...
function Signal:Connect(fn: (...any) -> ()): RBXScriptConnection
	local c = self._be.Event:Connect(fn)
...
function Signal:Once(fn: (...any) -> ()): RBXScriptConnection
	...
	conn = self._be.Event:Connect(function(...)
		conn:Disconnect()
		fn(...)
	end)
...
local Tokens = {
	...
	typography = {
		...
		label   = { font = Enum.Font.GothamMedium, size = 18 }, -- fixed: was GothamSemibold
		body    = { font = Enum.Font.Gotham,       size = 16 },
		small   = { font = Enum.Font.Gotham,       size = 14 },
	},
}

local function applyCorner(inst: Instance, radius: UDim?): UICorner
...
local function applyStroke(inst: Instance, color: Color3?, thickness: number?, transparency: number?): UIStroke
...
local function applyPadding(inst: Instance, p: { top: number?, bottom: number?, left: number?, right: number? }?): UIPadding
...
function ShopController:_passCard(pass: GamePass, parent: Instance, order: number)
	...
	local row = Frame({ BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 40), Parent = inner })
	local h = Instance.new("UIListLayout")
	h.FillDirection = Enum.FillDirection.Horizontal
	h.Padding = UDim.new(0, Tokens.spacing.xs)
	h.Parent = row
