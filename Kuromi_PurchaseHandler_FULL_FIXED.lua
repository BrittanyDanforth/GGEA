--[[
	✨ Kuromi Purchase Handler - FULL FIXED VERSION (matches HelloKitty)
...
print("🔍 [Kuromi] ========== FINDING TYCOON ID ==========")
print("🔍 [Kuromi] Script location:", script:GetFullName())
print("🔍 [Kuromi] script.Parent:", script.Parent.Name, "(" .. script.Parent.ClassName .. ")")
print("🔍 [Kuromi] script.Parent path:", script.Parent:GetFullName())

local TYCOON_ID = script.Parent:GetAttribute("TycoonId") or script.Parent.Name
print("🔍 [Kuromi] Selected TYCOON_ID:", TYCOON_ID)
...
if not datastoreSuccess then
	warn("[Kuromi] DataStore unavailable:", datastoreError)
end
...
local function saveAutoCollectPreference(player, enabled)
	...
	if not success then
		warn("[Kuromi] Failed to save auto-collect preference:", err)
	end
...
local function createMinimalParticles(position)
	...
	emitter.Color = ColorSequence.new(Color3.new(1, 1, 0.8)) -- Light blue for Kuromi
...
local function setupAutoCollect(player)
	...
	if giver then
		...
		if autoCollectEnabled[player] ~= false then
			frame.BackgroundColor3 = Color3.new(0, 0.7, 0.7)  -- Sea green
			frame.BackgroundTransparency = 0.2
			label.Text = "AUTO"
			label.TextColor3 = Color3.new(1, 1, 1)
		else
...
local function resetTycoonPurchases()
	print("🔄 [Kuromi] RESETTING PURCHASE HANDLER (MULTI-TYCOON SAFE MODE)...")
	...
	if CONFIG.USE_ATTRIBUTE_CLEANUP then
		...
		print("  ✓ [Kuromi] Destroyed", destroyedCount, "owned drops (attribute-based)")
	...
	elseif CONFIG.USE_BOUNDING_BOX_FALLBACK then
		...
		print("  ✓ [Kuromi] Destroyed", destroyedCount, "drops (bounding box fallback)")
	end
	...
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			...
			if model and model:IsA("Model") and (
				...
				model.Name:match("^Kuromi") or 
				model.Name:match("^WhiteHeart") or
				model.Name:match("^PremiumKuromi") or
...
print("✅ [Kuromi] Purchase Handler FULL FIXED VERSION loaded!")
print("🛡️ [Kuromi] Multi-Tycoon Safe: ENABLED")
print("🔧 [Kuromi] All features match HelloKitty handler")
