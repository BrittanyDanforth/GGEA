--[[
	✨ Kuromi Purchase Handler - LIVE SERVER FIXED
	
	🔧 LIVE SERVER FIXES:
	✅ PlayerRemoving hook - Forces cleanup when player leaves (doesn't rely on Owner changing)
	✅ PartStorage cleanup - Actually destroys dropper storage folder contents
	✅ GetDescendants() sweep - Finds all nested drops, not just top-level
	✅ GUID tracking - Prevents double-collection on network lag
	✅ Safe pcall wrapping - Player cleanup won't error if player already gone
	✅ Physics heartbeat wait - Lets pending touches finish before cleanup
...
local CONFIG = {
	...
	-- Cleanup
	CLEANUP_RADIUS = 120, -- Studs radius for drop cleanup
}
...
-- ========================================
-- COMPLETE RESET FUNCTION - LIVE SERVER FIXED!
-- ========================================
local function resetTycoonPurchases()
	print("🔄 [Kuromi] RESETTING PURCHASE HANDLER (LIVE SERVER MODE)...")
	...
	for _, dropper in ipairs(script.Parent:GetChildren()) do
		if dropper.Name:find("Dropper") then
			...
			print("  ⏸️ [Kuromi] Stopped dropper:", dropper.Name)
		end
	end
	...
	-- 🔧 LIVE FIX: Clear PartStorage folder (where DropperCore actually puts drops!)
	if partStorage then
	...
	-- 🔧 LIVE FIX: Use GetDescendants() and check ALL models/parts with Cash
	local destroyedCount = 0
	local tycoonPosition = script.Parent:GetPivot().Position
	local radius = CONFIG.CLEANUP_RADIUS
	...
	print("  ✓ [Kuromi] Destroyed", destroyedCount, "cash drops (world sweep)")
...
-- ========================================
-- INSTANT PART COLLECTOR - LIVE SERVER FIXED WITH GUID!
-- ========================================
for _, collector in ipairs(essentials:GetChildren()) do
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			-- 🔧 LIVE FIX: GUID-based tracking to prevent double-collection on network lag
			...
			if isModelDrop and modelCashValue > 0 then
				-- 🔧 LIVE FIX: Check GUID, not just model reference
...
-- ⚠️ NOTE: Remove this ProcessReceipt assignment!
-- Move to a central ServerScriptService script to avoid conflicts.
-- See: https://create.roblox.com/docs/production/monetization/developer-products
--[[
MarketplaceService.ProcessReceipt = function(receiptInfo)
	-- MOVE THIS TO CENTRAL SCRIPT!
end
--]]
...
print("✅ [Kuromi] Purchase Handler LIVE SERVER FIXED loaded!")
