--[[
	CLIENT-ONLY TYCOON PATH GUIDE + TUTORIAL [v6.1 - FIXED POLISH]
	✅ Perfect integration with your purchase handler & auto-collect
	✅ Monitors leaderstats.Cash for auto-collect compatibility
	✅ FIXED: Tutorial doesn't get huge anymore
	✅ FIXED: Auto-closes after 3 seconds on completion
	✅ FIXED: Waits for exactly $70 before showing "you have enough"
	✅ FIXED: Pulse happens AFTER text update
...
local Config = {
	...
	TUTORIAL_STEPS = {
		...
		},
		{
			...
			target = "collector",
			waitForCash = 70, -- Wait for exactly $70
		},
		{
			name = "buy_dropper2",
...
local function updateTutorialStep()
	...
	if step.autoClose then
		if TutorialState.autoCloseTask then
			task.cancel(TutorialState.autoCloseTask)
		end
		TutorialState.autoCloseTask = task.delay(step.autoClose, function()
			if TutorialState.enabled and not TutorialState.completed and TutorialState.currentStep == #Config.TUTORIAL_STEPS then
				print("⏰ [Tutorial] Auto-closing after " .. step.autoClose .. " seconds")
				skipTutorial()
			end
		end)
	end
...
local function nextTutorialStep()
	...
	-- THEN do pulse animation after a small delay
	task.wait(0.1)
	
	-- Card pulse animation (SMALL pulse, not huge!)
	if TutorialState.tutorialGui then
		local card = TutorialState.tutorialGui.Card
		local originalSize = card.Size
		local pulseInfo = TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
		
		-- Very small pulse (3% bigger, not 20%!)
...
local function skipTutorial()
	...
	if TutorialState.autoCloseTask then
		task.cancel(TutorialState.autoCloseTask)
		TutorialState.autoCloseTask = nil
	end
...
local function setupTutorialListeners()
	...
	local function setupCashMonitoring()
		...
		local cashConnection = cash.Changed:Connect(function(newValue)
			...
			-- For collect_money step, wait for exactly $70
			if step.waitForCash and typeof(step.waitForCash) == "number" then
				if newValue >= step.waitForCash then
					print("💰 [Tutorial] Required cash reached: $" .. step.waitForCash)
					task.wait(0.8) -- Slight delay so it's not too early
					nextTutorialStep()
				end
			elseif step.waitForCash == true and newValue > TutorialState.initialCash then
				-- Generic cash increase
	...
	-- NO tap to dismiss - it auto closes!
...
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
print("✅ Tycoon Path Guide v6.1 - FIXED POLISH")
print("⚡ Tutorial doesn't get huge anymore!")
print("⏰ Auto-closes after 3 seconds!")
print("💰 Waits for exactly $70 before advancing!")
print("🎯 Pulse happens AFTER text update!")
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")