--[[
    SANRIO SHOP — AAA CARDS (PHONE-READY)
    • Description text large & readable (TextScaled + constraint)
    • AAA card styling (inner panel, stroke, gradient, badges)
    • Mobile-friendly title-fitting
    • Fixed Lua issues: no ternary, no +=
    • 5 PHONE FIXES APPLIED:
      ✓ Smaller pills (Fit scaleType, min 54px)
      ✓ Dynamic content position (follows real tab height)
      ✓ Single column on phone portrait
      ✓ Bigger desc text (54px height)
      ✓ Safe area respect (DeviceSafeInsets)
]]
...
local CONTENT_TOP_Y = 0.472  -- base; computed dynamically in resize
local BAR_WIDTH_FACTOR = 0.94  -- FIX #1: was 0.86; more room for pills
local CONTENT_WIDTH_FACTOR = 0.90
local CONTENT_HEIGHT_FACTOR = 0.48

-- Top pill sizing (FIX #1: smaller for phones)
local CASH_H_FACTOR, GP_H_FACTOR = 0.075, 0.082  -- was 0.09, 0.095
local PILL_MIN_H, PILL_MAX_H = 54, 120            -- was 72, 120
local CASH_RATIO, GP_RATIO = 2.85, 3.60

-- ======== GRID / CARD ========
local GRID_X_SCALE = 0.455
local CARD_AR = 1.42
...
local DESC_HEIGHT = 54   -- FIX #4: taller so TextScaled can grow
local BTN_BOTTOM = -28
...
-- Smarter right pad so long titles don't over-shrink
...
-- Helper: bubble text with constraint (desc)
local function applyBubble(label, opts)
	opts = opts or {}
	label.TextWrapped = (opts.wrap ~= false)
	label.TextScaled = true
	label.TextColor3 = opts.fill or Color3.fromRGB(255,255,255)
	label.TextStrokeColor3 = opts.stroke or Color3.fromRGB(168,150,232)
	label.TextStrokeTransparency = opts.strokeT or 0.08
	label.Font = opts.font or Enum.Font.FredokaOne
	local tsc = label:FindFirstChildOfClass("UITextSizeConstraint") or Instance.new("UITextSizeConstraint")
	tsc.MinTextSize = opts.min or 18
	tsc.MaxTextSize = opts.max or 24
	tsc.Parent = label
end
...
local function buildDesc(content, product)
	...
	desc.Size = UDim2.new(DESC_WIDTH_SCALE, 0, 0, DESC_HEIGHT)
	desc.Text = product.description or ""
	desc.TextXAlignment = Enum.TextXAlignment.Left
	desc.TextYAlignment = Enum.TextYAlignment.Top
	desc.LineHeight = 1.08
	-- BIGGER description (phone 16–22, desktop 18–24)
	applyBubble(desc, {min = isPhone() and 16 or 18, max = isPhone() and 22 or 24, strokeT = 0.18})
	desc.Parent = content
	return desc
end
...
function Shop:initialize()
	...
	self:setupHandlers()
	print("[SanrioShop_AAA] 📱 Phone-ready version loaded!")
end
...
function Shop:makePill(name, imageId, ratio)
	...
	-- FIX #1: Fit instead of Crop prevents chopping
...
function Shop:createMainInterface()
	...
	self.gui = gui
	gui.Name = "SanrioShopMain"; gui.ResetOnSpawn = false; gui.DisplayOrder = 1000
	gui.Enabled = false
	-- FIX #5: Respect safe areas
	...
	dim.BackgroundTransparency = 0.38; dim.BorderSizePixel = 0; dim.Parent = gui
	...
	self.mainFrame.ZIndex = 1; self.mainFrame.Parent = gui
...
-- FIX #3: Single column on phone portrait
function Shop:_bindGridAspect(grid)
	local function recalc()
		...
		-- Smaller padding for single column
...
function Shop:createProductItem(product, productType, parent)
	...
	-- BUY button (FIX #4: shorter text on phones)
...
-- FIX #2: Dynamic content position based on real tab height
function Shop:setupDynamicSizing()
	local function resize()
		...
		-- Compute content Y from real tab height (prevent overlap on small screens)
		...
		local relY = (barBottom - frameTop + 16) / H  -- 16px gap
...
print("[SanrioShop_AAA] ✅ All 5 phone fixes applied!")
print("[SanrioShop_AAA] 📐 Pills: min="..PILL_MIN_H..", bar width="..BAR_WIDTH_FACTOR)
print("[SanrioShop_AAA] 📱 Single column on phone portrait")
print("[SanrioShop_AAA] 📏 Dynamic content Y (follows tab height)")
print("[SanrioShop_AAA] 🛡️ Safe area aware (DeviceSafeInsets)")
