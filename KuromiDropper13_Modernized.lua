--[[
	Kuromi Dropper 13 - MODERNIZED (Paintball Gun Mesh - Identical to Droppers 11 & 12)
	✅ Fixed: meshDrop set to true so mesh actually shows
...
-- Physics settings
local PHYSICS_DENSITY = 0.2
local PHYSICS_FRICTION = 0.6
local PHYSICS_ELASTICITY = 0.05

-- Visual settings
local DROP_SIZE = Vector3.new(0.2, 0.2, 0.2)

-- Mesh settings
local MESH_ID = "rbxasset://fonts/PaintballGun.mesh"
local TEXTURE_ID = "rbxasset://textures/PaintballGunTex128.png"
...
local function createDropEffects(part)
    ...
    light.Brightness = 1
    light.Range = 6
    light.Color = Color3.fromRGB(255, 100, 50) -- Orange for paintball gun
    light.Parent = part
...
local function createDrops()
    ...
    while true do
        ...
        part.Name = "PaintballDrop_" .. count
        ...
        -- Setup mesh (this was the main bug - meshDrop was false!)
...
if not success then
    warn("Error in Kuromi Dropper 13:", errorMsg)
end