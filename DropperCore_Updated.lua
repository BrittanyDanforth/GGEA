...
-- ✅ ALL KNOWN DROPPER GROUPS - Updated with HelloKitty groups!
local ALL_DROPPER_GROUPS = {
	...
	"HelloKittyDrops1", "HelloKittyDrops2", "HelloKittyDrops3", -- Added HelloKitty groups
	"Drops" -- Default group
}
...
function Core.Run(config)
	...
	while true do
		...
		-- Set velocity with random offset
		local velocity = spawnVelocity
		if config.randomVelocity then
			velocity = Vector3.new(ox * 2, spawnVelocity.Y, oz * 2)
		end
		...
		-- Add spawn time attribute if requested
		if config.addSpawnTime then
