--[[
    Sanrio Shop GUI — FIXED PILL BUTTONS (v3.5)
    
    ✅ FIXES:
    • Pill images now FILL the entire button (changed ScaleType to Stretch)
    • Removed broken resizeAll() code that tried to clear children
    • Clean resize logic that doesn't break tactile feedback
    • Buttons properly sized and visible from the start
...
--// Tabs region (lower & big)
local tabs = Instance.new("Frame")
tabs.Name = "Tabs"
tabs.BackgroundTransparency = 1
tabs.AnchorPoint = Vector2.new(0.5, 0)
tabs.Position = UDim2.fromScale(0.5, 0.35) -- lowered under scallops
tabs.Size = UDim2.fromScale(0.90, 0)       -- actual pixel height set in resize()
tabs.Parent = frame
...
content.Size = UDim2.fromScale(0.86, 0.40) -- resized dynamically too
...
--// ✅ FIX: Changed ScaleType to Stretch so image FILLS the button!
local function createPillButton(name: string, imageId: string): ImageButton
    ...
    btn.Image = imageId
    btn.ScaleType = Enum.ScaleType.Stretch  -- ✅ FIXED! Was Fit, now Stretch
    btn.Size = UDim2.fromOffset(300, 120)   -- placeholder, resized later
    btn.ZIndex = 10
    btn.Parent = tabs
...
--// ✅ CLEAN RESIZE LOGIC (no broken ClearAllChildren!)
local function resizeSafe()
    local H = frame.AbsoluteSize.Y
    if H <= 0 then return end

    -- Big tab pixels (clamped for mobile/desktop)
    local tabPx = clamp(math.floor(H * 0.14), 110, 180)
    local pillRatio = 3.9  -- pill aspect ratio (width = height * 3.9)
    local tabW = math.floor(tabPx * pillRatio)

    -- Update tab container and button sizes
    tabs.Size = UDim2.new(0, math.floor(H * 0.86), 0, tabPx)
    cashTab.Size = UDim2.fromOffset(tabW, tabPx)
    gpTab.Size   = UDim2.fromOffset(tabW, tabPx)

    -- Content panel height scales with body
    content.Size = UDim2.new(0, math.floor(H * 0.86), 0, math.floor(H * 0.42))
end
...
print("✅ [SanrioShop] FIXED - Pill buttons now fill with image!")
