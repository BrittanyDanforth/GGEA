...
print("💰 [MONEYSHOPSERV] Initializing...")
...
task.spawn(function()
	...
	if _G.AddPlayerMoney then
		print("✅ [MONEYSHOPSERV] Connected to Global Money API (_G.AddPlayerMoney)")
	else
		warn("⚠️ [MONEYSHOPSERV] _G.AddPlayerMoney not available after 30s.")
		warn("⚠️ [MONEYSHOPSERV] Will use fallback methods (leaderstats or ServerStorage)")
	end
end)
...
local function grantCashToPlayer(userId: number, amount: number): boolean
	...
	if not player then 
		warn("❌ [MONEYSHOPSERV] Player not found for userId:", userId)
		return false 
	end

	print(string.format("💵 [MONEYSHOPSERV] Granting %d cash to %s...", amount, player.Name))
	...
	if _G.AddPlayerMoney then
		...
		if ok then
			print(string.format("✅ [MONEYSHOPSERV] Granted via _G.AddPlayerMoney: %d to %s", amount, player.Name))
			return true
		end
		warn("[MONEYSHOPSERV] _G.AddPlayerMoney failed:", err)
	end
	...
	if leaderstats then
		...
		if cashValue and cashValue:IsA("IntValue") then
			...
			print(string.format("✅ [MONEYSHOPSERV] Granted via leaderstats: %d to %s", amount, player.Name))
			return true
		end
	end
	...
	if not folder then
		...
		print("📁 [MONEYSHOPSERV] Created ServerStorage.PlayerMoney folder")
	end
	...
	print(string.format("✅ [MONEYSHOPSERV] Granted via ServerStorage: %d to %s", amount, player.Name))
	return true
end
...
local function processReceipt(receiptInfo)
	...
	if not player then
		warn(string.format("⚠️ [MONEYSHOPSERV] Player not in game for receipt %s, will retry", receiptInfo.PurchaseId))
		return Enum.ProductPurchaseDecision.NotProcessedYet
	end
	...
	if not ok then
		warn("[MONEYSHOPSERV] DataStore error for receipt:", stateOrErr)
		return Enum.ProductPurchaseDecision.NotProcessedYet
	end
	
	if stateOrErr == "granted" then
		-- Already processed this purchase
		print(string.format("✓ [MONEYSHOPSERV] Receipt %s already granted (idempotent)", receiptInfo.PurchaseId))
	...
	if not granted then
		warn("[MONEYSHOPSERV] Failed to grant currency, will retry")
		return Enum.ProductPurchaseDecision.NotProcessedYet
	end
	...
	print(string.format("🎉 [MONEYSHOPSERV] Purchase complete! Granted %d cash to %s (productId: %d)", 
...
do
	...
	grantRemote.OnServerEvent:Connect(function(player, productId)
		...
		if amount then
			...
			print(string.format("🧪 [MONEYSHOPSERV] Studio test grant: %d cash to %s (product %d)", 
				amount, player.Name, productId))
		end
	end)
	
	print("🧪 [MONEYSHOPSERV] Studio testing fallback enabled")
end
