--[[
	✨ MyMelody Purchase Handler - PRODUCTION VERSION
...
local CONFIG = {
	...
	COLLECTOR_IDLE_COLOR = BrickColor.new("Hot pink"),
...
print("🏠 [MyMelody] Tycoon ID:", TYCOON_ID)
...
local ownershipCache, OWNERSHIP_CACHE_TTL = {}, 45

local AUTO_COLLECT_STORE_NAME = "MyMelodyAutoCollect_v1"
...
autoCollectToggle.Name, autoCollectToggle.Parent = "AutoCollectToggle", remotesFolder
local moneyCollectRemote = remotesFolder:FindFirstChild("MoneyCollected") or Instance.new("RemoteEvent")
moneyCollectRemote.Name, moneyCollectRemote.Parent = "MoneyCollected", remotesFolder
local gamepassPurchaseRemote = remotesFolder:FindFirstChild("GamepassPurchased") or Instance.new("RemoteEvent")
gamepassPurchaseRemote.Name, gamepassPurchaseRemote.Parent = "GamepassPurchased", remotesFolder
...
local function playSound(part, soundId, volume)
	...
	sound.SoundId, sound.Volume, sound.Parent = "rbxassetid://" .. tostring(soundId), volume or 0.3, part
	sound:Play()
...
local function createMinimalParticles(position)
	...
	attachment.Position, attachment.Parent = position, workspace.Terrain
	local emitter = Instance.new("ParticleEmitter")
	emitter.Texture, emitter.Rate, emitter.Lifetime = "rbxasset://textures/particles/sparkles_main.dds", 30, NumberRange.new(0.3, 0.5)
	emitter.VelocityInheritance, emitter.EmissionDirection, emitter.Speed = 0, Enum.NormalId.Top, NumberRange.new(3, 5)
	emitter.SpreadAngle, emitter.Color, emitter.Size = Vector2.new(15, 15), ColorSequence.new(Color3.new(1, 0.75, 0.8)), NumberSequence.new(0.3)
	emitter.Parent = attachment
	task.wait(0.1) emitter.Enabled = false Debris:AddItem(attachment, 1)
end

local function check2xCashOwnership(player)
	if RunService:IsStudio() and _G.StudioGamepassPurchases then
...
local function update2xCashIndicator(giver, has2xCash)
	...
	if has2xCash and not indicator2x then
		local bb = Instance.new("BillboardGui") bb.Name, bb.MaxDistance, bb.Size, bb.StudsOffset, bb.AlwaysOnTop, bb.Parent = "2xCashIndicator", 100, UDim2.new(3, 0, 1, 0), Vector3.new(0, 5, 0), true, giver
		local f = Instance.new("Frame") f.Size, f.BackgroundColor3, f.BackgroundTransparency, f.BorderSizePixel, f.Parent = UDim2.new(1, 0, 1, 0), Color3.fromRGB(255, 192, 203), 0.2, 0, bb
		local c = Instance.new("UICorner") c.CornerRadius, c.Parent = UDim.new(0.2, 0), f
		local l = Instance.new("TextLabel") l.Size, l.BackgroundTransparency, l.Text, l.TextScaled, l.TextColor3, l.Font, l.TextStrokeTransparency, l.TextStrokeColor3, l.Parent = UDim2.new(1, 0, 1, 0), 1, "2X CASH", true, Color3.new(1, 1, 1), Enum.Font.SourceSansBold, 0, Color3.new(0, 0, 0), f
...
local function setupAutoCollect(player)
	...
	print("💕 [MyMelody] Auto-Collect activated for", player.Name)
	...
	if giver then
		...
		local ai = Instance.new("BillboardGui") ai.Name, ai.MaxDistance, ai.Size, ai.StudsOffset, ai.AlwaysOnTop, ai.Parent = "AutoCollectIndicator", 100, UDim2.new(3, 0, 0.8, 0), Vector3.new(0, 3.5, 0), true, giver
		local f = Instance.new("Frame") f.Size, f.BorderSizePixel, f.Parent = UDim2.new(1, 0, 1, 0), 0, ai
		local c = Instance.new("UICorner") c.CornerRadius, c.Parent = UDim.new(0.2, 0), f
		local l = Instance.new("TextLabel") l.Size, l.BackgroundTransparency, l.TextScaled, l.Font, l.TextStrokeTransparency, l.TextStrokeColor3, l.Parent = UDim2.new(1, 0, 1, 0), 1, true, Enum.Font.SourceSansBold, 0, Color3.new(0, 0, 0), f
		if autoCollectEnabled[player] ~= false then
			f.BackgroundColor3, f.BackgroundTransparency, l.Text, l.TextColor3 = Color3.fromRGB(255, 182, 193), 0.2, "AUTO", Color3.new(1, 1, 1)
		else
			f.BackgroundColor3, f.BackgroundTransparency, l.Text, l.TextColor3 = Color3.new(0.5, 0, 0), 0.2, "AUTO X", Color3.new(1, 0.3, 0.3)
		end
	end
end
...
autoCollectToggle.OnServerEvent:Connect(function(player, enabled)
	...
	if giver then
		...
		if indicator then
			...
			if label then
				if enabled then
					label.Text, label.TextColor3, frame.BackgroundColor3 = "AUTO", Color3.new(1, 1, 1), Color3.fromRGB(255, 182, 193)
				else
					label.Text, label.TextColor3, frame.BackgroundColor3 = "AUTO X", Color3.new(1, 0.3, 0.3), Color3.new(0.5, 0, 0)
				end
			end
		end
	end
...
local function addSimpleHoverEffect(button)
	...
	local detector = Instance.new("Part") detector.Name, detector.Size, detector.Transparency, detector.CanCollide, detector.CFrame, detector.Parent = "HoverDetector", head.Size * 1.3, 1, false, head.CFrame, button
...
local function resetTycoonPurchases()
	...
	if partStorage then
		...
		print("  ✓ [MyMelody] Cleared PartStorage:", storageCount)
	end
	if CONFIG.USE_ATTRIBUTE_CLEANUP then
		...
		print("  ✓ [MyMelody] Destroyed", destroyedCount, "owned drops")
	elseif CONFIG.USE_BOUNDING_BOX_FALLBACK then
		...
		print("  ✓ [MyMelody] Destroyed", destroyedCount, "drops")
	end
	Money.Value = 0
...
tycoonOwner.Changed:Connect(function()
	...
	elseif newOwner ~= nil and currentOwner == nil then
		...
		if not spawnPart:IsA("SpawnLocation") then
			...
			s.Name, s.Size, s.CFrame, s.Anchored, s.CanCollide, s.BrickColor, s.TeamColor, s.Parent = "Spawn", spawnPart.Size, spawnPart.CFrame, true, spawnPart.CanCollide, spawnPart.BrickColor, TeamColor, essentials
			spawnPart:Destroy()
			spawnPart = s
		end
		spawnPart.Neutral, spawnPart.TeamColor, spawnPart.Enabled, spawnPart.Duration = false, TeamColor, true, 0
		-- ✅ FIX: Team assignment with proper newlines
...
for _, collector in ipairs(essentials:GetChildren()) do
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			...
			if model and model:IsA("Model") and (model.Name:match("^Drop_") or model.Name:match("^MyMelody") or model.Name:match("^PinkHeart") or model.Name:match("^PremiumMyMelody")) then
...
giver.Touched:Connect(function(hit)
	...
	if script.Parent.Owner.Value == player then
		...
		if playerStats and Money.Value > 0 then
			...
			local tl = Instance.new("TextLabel") tl.Size, tl.BackgroundTransparency, tl.Text, tl.TextScaled, tl.TextColor3, tl.Font, tl.TextStrokeTransparency, tl.Parent = UDim2.new(1, 0, 1, 0), 1, "+$" .. tostring(finalAmount) .. (has2x and " (2X!)" or ""), true, has2x and Color3.fromRGB(255, 192, 203) or Color3.new(0, 1, 0), Enum.Font.SourceSansBold, 0, bb
	...
	if objectName and Objects[objectName] then
		...
		if objectName:find("Door") or objectName:find("door") then
			for _, part in ipairs(newObject:GetDescendants()) do
				if part:IsA("BasePart") then part.BrickColor = BrickColor.new("Pink") if part.Material == Enum.Material.Neon then part.Material = Enum.Material.SmoothPlastic end end
			end
		end
...
print("✅ [MyMelody] PRODUCTION VERSION loaded!")
print("🛡️ Multi-Tycoon Safe | 🔧 Live Server Fixed | 🎮 All Features")
