...
local function getTycoonId(dropperModel)
	-- Try common tycoon ancestor names
	...
	if tycoon then
		...
		if id then
			...
			warn("[DropperCore] TycoonId attribute not found on", tycoon.Name, "- using Name as fallback")
			return tycoon.Name
		end
	end
	
	warn("[DropperCore] Could not find parent tycoon for dropper:", dropperModel.Name)
	return "Unknown"
end
...
function Core.Run(config)
	...
	print("🏠 [DropperCore.Run] Dropper belongs to tycoon:", TYCOON_ID)
...
function Core.RunModel(config)
	...
	print("🏠 [DropperCore.RunModel] Model dropper belongs to tycoon:", TYCOON_ID)
	...
	local function spawnOne(isPrewarm)
		...
		primary.Touched:Connect(function(hit)
			if isCollectorPart(hit, collectorNames, collectorTags) then collect() end
		end)
		...
		if LIFETIME and LIFETIME > 0 then
			task.delay(LIFETIME, function()
				if not collected then collect() end
			end)
		end
	end
