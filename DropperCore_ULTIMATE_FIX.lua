--!strict
-- DropperCore v4.3 — ULTIMATE FIX: Parent Lock Issue + Collection Issues + FULL DEBUG
...
function Pool:release(item: PoolItem)
	-- ULTIMATE FIX: Complete cleanup with multiple safety checks
	local function safeUnparent(model: Model)
		local attempts = 0
		while model.Parent and attempts < 10 do
	...
	-- ULTIMATE FIX: Force unparent with retries
	safeUnparent(item.model)
	...
	-- ULTIMATE FIX: Final safety check
	safeUnparent(item.model)
...
function Core.RunModel(config: {
	...
	while true do
		...
		-- ULTIMATE FIX: Create a completely fresh model instead of reusing pooled ones
		-- This prevents the parent lock issue entirely
		local freshModel = template:Clone()
		local freshItem = buildItemFromClone(freshModel, GROUP, DENSITY, FRICTION, ELASTICITY, CASH_VALUE, CASH_ON)
		freshItem.model.Name = NAME_PREFIX .. tostring(count)

		-- Use the fresh item instead of the pooled one
		item = freshItem
		...
		-- ULTIMATE FIX: Fresh models should never have parent lock issues
		...
		local function collectAndReturn()
			...
			tweenSubset(item, 1, math.max(FADE_TIME * 0.75, 0.2), 10, true, function()
				-- ULTIMATE FIX: Just destroy the model instead of trying to pool it
				-- This completely eliminates the parent lock issue
				item.model:Destroy()
			end)
		end
...
function Core.Run(_config)
	error("Core.Run not implemented in v4.3; use RunModel() for model droppers.")
end
