--[[
	DropperCore v4.7 - With Full Model Support
	✅ Supports cloning entire models as drops (not just meshes)
	...
	✅ Both Core.Run() and Core.RunModel() fully implemented
	✅ Fade in/out effects
...
-- Fade in effect for parts
local function fadeIn(obj, t, to)
...
-- Get primary part of model or the part itself
...
-- Set collision group for all parts in model
...
local function setPhysicsProperties(obj, density, friction, elasticity)
	local props = PhysicalProperties.new(density, friction, elasticity, 0.5, 0.5)
	
	if obj:IsA("BasePart") then
		obj.CustomPhysicalProperties = props
	elseif obj:IsA("Model") then
		for _, part in ipairs(obj:GetDescendants()) do
			if part:IsA("BasePart") then
...
local function scaleModel(model, scaleFactor)
	...
	for _, part in ipairs(model:GetDescendants()) do
		if part:IsA("BasePart") and part ~= primary then
			...
			local offset = primaryCF:PointToObjectSpace(part.Position)
			part.CFrame = primaryCF * CFrame.new(offset * scaleFactor) * (part.CFrame - part.Position)
...
-- ✅ Core.Run() - Creates drops from scratch
...
function Core.RunModel(config)
	...
	-- config.model: the dropper model containing a child "Drop" (BasePart) to spawn from
	-- config.partStorage: Folder where drops go
	-- config.templateModel: Model to clone as the drop
	...
	local GROUP = config.dropGroup or "Drops"
	local PLAYER_GRP = config.playerGroup or "Players"
	local SPAWN_Y_OFF = -(config.extraLower or 0) - 2.5
	local FADE_TIME = config.fadeTime or 0.3
	local DENSITY = config.density or 0.05
	local FRICTION = config.friction or 0.2
	local ELASTICITY = config.elasticity or 0.0
	...
	while true do
		...
		local dropModel = templateModel:Clone()
		dropModel.Name = (config.namePrefix or "Drop_") .. count

		-- Scale the model if needed
		if SCALE_FACTOR ~= 1 then
			scaleModel(dropModel, SCALE_FACTOR)
		end

		-- Get primary part for positioning
		local primary = getPrimaryPart(dropModel)
		if not primary then
		...
		-- Store original transparencies for fade effects
		...
		setCollisionGroup(dropModel, GROUP)

		-- Set physics properties
		setPhysicsProperties(dropModel, DENSITY, FRICTION, ELASTICITY)
		...
		if dropModel:IsA("Model") and dropModel.PrimaryPart then
			dropModel:SetPrimaryPartCFrame(CFrame.new(pos) * rotation)
		else
		...
		if CASH_ON == "all" then
			...
			cash.Name = "Cash"
			cash.Value = CASH_VALUE
			cash.Parent = cashPart
		end

		-- Fade in effect
		fadeIn(dropModel, FADE_TIME, 0)

		-- Add to world
		dropModel.Parent = storage
		...
		if dropModel:IsA("Model") then
			...
			for _, part in ipairs(dropModel:GetDescendants()) do
				...
				elseif part:IsA("SpecialMesh") then
					originalSizes[part] = part.Scale
					part.Scale = part.Scale * startScale
				end
			end
			
			-- Animate to original sizes
		...
		local function onTouch(hit)
			...
			-- Anchor and disable collision for all parts
			for _, part in ipairs(dropModel:GetDescendants()) do
				if part:IsA("BasePart") then
					part.Anchored = true
					part.CanTouch = false
			...
			-- Fade out
			fadeOut(dropModel, FADE_TIME * 0.7)
		...
		-- Lifetime cleanup
		Debris:AddItem(dropModel, LIFETIME)
	end
end
