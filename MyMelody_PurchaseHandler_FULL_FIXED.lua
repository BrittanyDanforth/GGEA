--[[
	✨ MyMelody Purchase Handler - FULL FIXED VERSION (matches HelloKitty)
	
	🛡️ MULTI-TYCOON SAFETY:
	✅ Attribute-based cleanup - Only destroys MyMelody's drops
...
-- 🛡️ MULTI-TYCOON: Get this tycoon's ID
print("🔍 [MyMelody] ========== FINDING TYCOON ID ==========")
print("🔍 [MyMelody] Script location:", script:GetFullName())
print("🔍 [MyMelody] script.Parent:", script.Parent.Name, "(" .. script.Parent.ClassName .. ")")
print("🔍 [MyMelody] script.Parent path:", script.Parent:GetFullName())

local TYCOON_ID = script.Parent:GetAttribute("TycoonId") or script.Parent.Name
print("🔍 [MyMelody] Selected TYCOON_ID:", TYCOON_ID)
...
if not datastoreSuccess then
	warn("[MyMelody] DataStore unavailable:", datastoreError)
end
...
local function setOwnershipCache(userId, passId, value)
	ownershipCache[getCacheKey(userId, passId)] = {
		value = value,
		timestamp = os.clock()  -- 🔧 Use os.clock() instead of tick()
	}
end
...
local function getOwnershipCache(userId, passId)
	...
	-- 🔧 Clear expired cache to prevent growth
...
local function saveAutoCollectPreference(player, enabled)
	...
	if not success then
		warn("[MyMelody] Failed to save auto-collect preference:", err)
	end
...
local function createMinimalParticles(position)
	...
	emitter.Color = ColorSequence.new(Color3.new(1, 0.75, 0.8)) -- Light blue for MyMelody
...
local function cleanupAutoCollect(player)
	...
	print("🎵 [MyMelody] Auto-Collect deactivated for", player and player.Name or "unknown")
end
...
autoCollectToggle.OnServerEvent:Connect(function(player, enabled)
	...
	print("🎚️ [MyMelody] Auto-Collect", enabled and "enabled" or "disabled", "for", player.Name)
end)
...
-- 🔧 OPTIMIZED: Hover effect only checks touching character
...
local function resetTycoonPurchases()
	print("🔄 [MyMelody] RESETTING PURCHASE HANDLER (MULTI-TYCOON SAFE MODE)...")
	...
	for _, dropper in ipairs(script.Parent:GetChildren()) do
		if dropper.Name:find("Dropper") then
			...
			print("  ⏸️ [MyMelody] Stopped dropper:", dropper.Name)
		end
	end
	...
	if partStorage then
		...
		print("  ✓ [MyMelody] Cleared PartStorage:", storageCount, "drops")
	end
	...
	if CONFIG.USE_ATTRIBUTE_CLEANUP then
		...
		print("  ✓ [MyMelody] Destroyed", destroyedCount, "owned drops (attribute-based)")
	...
	elseif CONFIG.USE_BOUNDING_BOX_FALLBACK then
		...
		print("  ✓ [MyMelody] Destroyed", destroyedCount, "drops (bounding box fallback)")
	end
...
Players.PlayerRemoving:Connect(function(player)
	if tycoonOwner.Value == player then
		print("👋 [MyMelody] PLAYER REMOVING:", player.Name, "- FORCING CLEANUP")
...
tycoonOwner.Changed:Connect(function()
	...
	if newOwner == nil and currentOwner ~= nil then
		...
		print("👋 [MyMelody] Owner left (via Owner.Changed), resetting purchases...")
		resetTycoonPurchases()
		currentOwner = nil
	...
	elseif newOwner ~= nil and currentOwner == nil then
		...
		for _, dropper in ipairs(script.Parent:GetChildren()) do
			if dropper.Name:find("Dropper") then
				...
				if dropperScript then
					...
					print("  ▶️ [MyMelody] Started dropper:", dropper.Name)
				end
			end
		end
		...
		print("🔍 [MyMelody] Checking if", newOwner.Name, "owns auto-collect gamepass...")
	...
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			...
			if model and model:IsA("Model") and (
				...
				model.Name:match("^PinkHeart") or
				model.Name:match("^PremiumMyMelody") or
...
print("✅ [MyMelody] Purchase Handler FULL FIXED VERSION loaded!")
print("🛡️ [MyMelody] Multi-Tycoon Safe: ENABLED")
print("🔧 [MyMelody] All features match HelloKitty handler")
print("🎵 [MyMelody] Auto-Collect Gamepass ID:", CONFIG.AUTO_COLLECT_GAMEPASS_ID)
print("💵 [MyMelody] 2x Cash Gamepass ID:", CONFIG.DOUBLE_CASH_GAMEPASS_ID)
