--[[
    ...
    2. Create a folder next to this script (same parent)
    3. Place SanrioShop_Core and SanrioShop_UI as ModuleScripts in that folder
    4. Update the require paths below to match your structure
    
    This version fixes:
    1. Layout property error
    2. Sound asset loading
    3. Product visibility issues
    4. Modular architecture for better maintainability
--]]
...
-- MODULE IMPORTS
-- ========================================
-- IMPORTANT: Adjust these paths to match your Roblox hierarchy
-- Example: If modules are in ReplicatedStorage.ShopModules, use:
-- local Core = require(ReplicatedStorage:WaitForChild("ShopModules"):WaitForChild("SanrioShop_Core"))
-- local UI = require(ReplicatedStorage:WaitForChild("ShopModules"):WaitForChild("SanrioShop_UI"))

-- Default: assumes modules are in a folder next to this script
local Core = require(script.Parent:WaitForChild("SanrioShop_Core"))
local UI = require(script.Parent:WaitForChild("SanrioShop_UI"))

-- Link UI module with Core for cross-module functionality
UI.setCore(Core)
...
function Shop:refreshAllProducts()
	-- Clear ownership cache to force fresh checks
	for _, pass in ipairs(Core.DataManager.products.gamepasses) do
		local cacheKey = Player.UserId .. "_" .. pass.id
		Core.Cache.new(Core.CONSTANTS.CACHE_OWNERSHIP):clear(cacheKey)
...
print("[SanrioShop] Version:", Core.VERSION)
print("[SanrioShop] Press 'M' to open the shop")
