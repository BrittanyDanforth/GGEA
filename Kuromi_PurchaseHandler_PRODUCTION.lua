--[[
	✨ Kuromi Purchase Handler - PRODUCTION VERSION
...
-- 🔧 OPTIMIZED: Hover effect checks only touching character
	...
	detector.Touched:Connect(function(hit)
		...
		if humanoid and not isHovering then
			...
			task.spawn(function()
				while isHovering do
					task.wait(0.1)
					-- Only check the character that touched
...
local function resetTycoonPurchases()
	print("🔄 [Kuromi] RESETTING (MULTI-TYCOON SAFE MODE)...")
	...
	elseif CONFIG.USE_BOUNDING_BOX_FALLBACK then
		...
		-- ✅ FIX: OverlapParams uses OverlapFilterType, not RaycastFilterType!
...
Players.PlayerRemoving:Connect(function(player)
	if tycoonOwner.Value == player then
		...
		-- 🔧 LIVE FIX: Safe player property cleanup
		pcall(function()
...
tycoonOwner.Changed:Connect(function()
	...
	elseif newOwner ~= nil and currentOwner == nil then
		...
		-- ✅ FIX: Proper syntax with newlines
	...
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			...
			if isModelDrop and modelCashValue > 0 then
				-- 🔧 GUID-based tracking to prevent double-collection
...
-- ⚠️ NOTE: Remove this ProcessReceipt assignment if you have multiple tycoons!
