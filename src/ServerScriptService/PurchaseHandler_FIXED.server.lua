--[[
	✨ ULTRA POLISHED Purchase Handler v2.2 - WHEEL BOOST INTEGRATION
	
	🎯 NEW: Now supports BOTH 2X Cash sources:
	   1. Permanent 2X Cash Gamepass
	   2. Temporary Wheel Boost (TwoXUntil attribute)
	
	CRITICAL FIX: applyMoneyMultiplier now checks player attributes for wheel boosts
--]]
...
-- ========================================
-- 2X CASH FUNCTIONS - NOW WITH WHEEL BOOST SUPPORT! 🎡
-- ========================================
...
-- 🎯 FIXED: Now checks BOTH permanent gamepass AND temporary wheel boost!
local function applyMoneyMultiplier(player, amount)
	...
	-- Apply 2X if EITHER is active (don't stack to 4X - that would be OP!)
	...
	if wasPurchased then
		...
		elseif gamePassId == CONFIG.DOUBLE_CASH_GAMEPASS_ID then
			...
			if script.Parent.Owner.Value == player then
				local giver = essentials:FindFirstChild("Giver")
				if giver then
					-- Check if wheel boost is also active
					local untilTs = player:GetAttribute("TwoXUntil")
					local hasWheelBoost = (typeof(untilTs) == "number" and untilTs > os.time())
...
connections.owner = tycoonOwner.Changed:Connect(function()
	...
	elseif newOwner ~= nil and currentOwner == nil then
		...
		local giver = essentials:FindFirstChild("Giver")
		if giver then
			-- Check BOTH sources of 2X
			local untilTs = newOwner:GetAttribute("TwoXUntil")
			local hasWheelBoost = (typeof(untilTs) == "number" and untilTs > os.time())
			local hasGamepass = check2xCashOwnership(newOwner)
			update2xCashIndicator(giver, hasGamepass or hasWheelBoost)
		end
...
giver.Touched:Connect(function(hit)
	...
	if script.Parent.Owner.Value == player then
		...
		if playerStats and Money.Value > 0 then
			...
			-- Apply 2x multiplier if owned (checks BOTH gamepass AND wheel boost!)
...
-- ========================================
print("✅ ULTRA POLISHED Purchase Handler v2.2 - WHEEL BOOST INTEGRATION loaded!")
print("🎯 NEW FEATURES:")
print("  1. ✅ Now checks BOTH 2X Cash gamepass AND wheel boost (TwoXUntil)")
print("  2. ✅ Real-time indicator updates when wheel boost activates/expires")
print("  3. ✅ Seamless integration - no conflicts between boost sources")
print("  4. ⚡ INSTANT CASH COLLECTION - No delays, immediate UI updates!")
...
print("🎡 Wheel Boost Support: ENABLED (checks TwoXUntil attribute)")
