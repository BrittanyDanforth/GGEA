--[[
	CLIENT-ONLY TYCOON PATH GUIDE + TUTORIAL [v4.5-ULTRA-POLISHED-FIXED]
	✅ Auto-Collect Compatible (monitors leaderstats.Cash!)
	✅ Built-in tutorial with cute, specific messages
	✅ FIXED: Proper fade out animations
	✅ FIXED: Text transition bugs
	✅ FIXED: Timing and polish issues
...
local Config = {
	...
	-- 🎓 TUTORIAL SETTINGS (FIXED TIMING)
	TUTORIAL_ENABLED = true,
	TUTORIAL_STEP_DELAY = 0.5, -- Delay between steps
	TUTORIAL_TEXT_TRANSITION_TIME = 0.3, -- Text fade transition
	TUTORIAL_STEPS = {
		{
			name = "claim_gate",
...
local TutorialState = {
	...
	tutorialGui = nil,
	skipButton = nil,
	isTransitioning = false, -- FIXED: Prevents overlapping transitions
	connections = {}, -- FIXED: Track all connections for cleanup
}
...
local function createHighlight(target)
	-- FIXED: Properly cleanup old highlight
	...
	-- FIXED: Use a connection we can cleanup
...
local function updateTutorialText(newTitle, newBody)
	...
	-- Fade out
	local fadeOutTween1 = TweenService:Create(titleLabel, 
		TweenInfo.new(Config.TUTORIAL_TEXT_TRANSITION_TIME/2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), 
		{TextTransparency = 1}
	)
	local fadeOutTween2 = TweenService:Create(bodyLabel, 
		TweenInfo.new(Config.TUTORIAL_TEXT_TRANSITION_TIME/2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), 
		{TextTransparency = 1}
	)
	...
	task.wait(Config.TUTORIAL_TEXT_TRANSITION_TIME/2)
	...
	local fadeInTween1 = TweenService:Create(titleLabel, 
		TweenInfo.new(Config.TUTORIAL_TEXT_TRANSITION_TIME/2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), 
		{TextTransparency = 0}
	)
	local fadeInTween2 = TweenService:Create(bodyLabel, 
		TweenInfo.new(Config.TUTORIAL_TEXT_TRANSITION_TIME/2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), 
		{TextTransparency = 0}
	)
...
local function updateTutorialStep()
	...
	task.wait(Config.TUTORIAL_STEP_DELAY) -- Give time for text transition
...
local function nextTutorialStep()
	...
	-- FIXED: Prevent overlapping transitions
...
local function skipTutorial()
	...
	-- FIXED: Proper fade out animation
	if TutorialState.tutorialGui then
		local card = TutorialState.tutorialGui:FindFirstChild("Card")
		if card then
			-- Fade out all elements
			local tweenInfo = TweenInfo.new(Config.FADE_OUT_TIME, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
			
			-- Fade card
			local fadeTween = TweenService:Create(card, tweenInfo, {
...
local function setupTutorialListeners()
	...
	if TutorialState.skipButton then
		local skipConnection = TutorialState.skipButton.Activated:Connect(skipTutorial)
...
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
print("✅ Tycoon Path Guide v4.5-ULTRA-POLISHED-FIXED")
print("⚡ FIXED: Proper fade animations!")
print("⚡ FIXED: Text transition bugs!")
