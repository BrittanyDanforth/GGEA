--!strict
-- DropperCore v4.5 — Enhanced collision handling
...
local function setupCollisionGroups(dropGroup: string, playerGroup: string)
	...
	-- Key fixes:
	-- 1. Drops don't collide with players
	pcall(function() PhysicsService:CollisionGroupSetCollidable(dropGroup, playerGroup, false) end)
	
	-- 2. Drops don't collide with each other (prevents stacking)
	pcall(function() PhysicsService:CollisionGroupSetCollidable(dropGroup, dropGroup, false) end)
	
	-- 3. ENSURE drops collide with Default group (conveyors, world, etc)
	pcall(function() PhysicsService:CollisionGroupSetCollidable(dropGroup, "Default", true) end)
end
...
local function createFreshDropModel(
	...
	-- Setup all parts with proper collision
	for _, part in ipairs(model:GetDescendants()) do
		if part:IsA("BasePart") then
			part.Anchored = false
			part.CanTouch = true
			part.CanQuery = true
			part.CanCollide = true  -- MUST be true to collide with conveyors
...
function Core.RunModel(config: {
	...
	while true do
		...
		local patternOffset = patterns[(count % #patterns) + 1]
		
		local offsetX = patternOffset.X + (math.random(-2, 2) * 0.05)
		local offsetZ = patternOffset.Z + (math.random(-2, 2) * 0.05)
		...
		-- Apply slight spread to prevent stacking
		primary.AssemblyLinearVelocity = Vector3.new(offsetX * 2, -8, offsetZ * 2)
		...
		local touchConn = primary.Touched:Connect(function(hit)
			...
			if isCollectorPart(hit, collectorNames, collectorTags) then
				...
				if cashVal then
					-- Cash detected, proceed with collection
				else
