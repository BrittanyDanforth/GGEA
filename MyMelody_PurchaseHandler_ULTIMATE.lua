--[[
	✨ MyMelody Purchase Handler - ULTIMATE FIXED with ALL FEATURES
	
	NEW FEATURES (from Kuromi):
	...
	✅ FIXED: Safe cleanup when players leave
--]]
...
-- ========================================
local CONFIG = {
	-- Gamepass IDs - UPDATE THESE FOR YOUR GAME!
	AUTO_COLLECT_GAMEPASS_ID = 1234567890, -- REPLACE WITH YOUR AUTO-COLLECT GAMEPASS ID
	DOUBLE_CASH_GAMEPASS_ID = 1234567891,  -- REPLACE WITH YOUR 2X CASH GAMEPASS ID
...
local function createMinimalParticles(position)
	...
	emitter.Color = ColorSequence.new(Color3.new(1, 0.75, 0.8)) -- Pink for MyMelody
...
local function update2xCashIndicator(giver, has2xCash)
	...
	if has2xCash then
		if not indicator2x then
			...
			frame.BackgroundColor3 = Color3.fromRGB(255, 192, 203) -- Pink
...
local function resetTycoonPurchases()
	...
	print("  ✓ [MyMelody] Destroyed", destroyedCount, "cash drops (models + parts)")
...
for _, collector in ipairs(essentials:GetChildren()) do
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			...
			if model and model:IsA("Model") and (model.Name:match("^Drop_") or model.Name:match("^MyMelodyDrop") or model.Name:match("^PinkHeart") or model.Name:match("^PremiumMyMelody")) then
				isModelDrop = true
...
giver.Touched:Connect(function(hit)
	...
	if script.Parent.Owner.Value == player then
		...
		if playerStats and Money.Value > 0 then
			...
			textLabel.TextColor3 = has2x and Color3.fromRGB(255, 192, 203) or Color3.new(0, 1, 0)
...
print("✅ [MyMelody] Purchase Handler ULTIMATE FIXED loaded with ALL FEATURES!")
...
print("🏠 [MyMelody] Spawn Location System: ENABLED")
print("🛡️ [MyMelody] Safe Cleanup System: ENABLED")
