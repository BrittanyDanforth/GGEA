...
local EXTRA_LOWER = 0.35   -- ⬅️ spawn lower (meters) - increased from 0.15
...
while true do
	...
	-- ⬇️ spawn lower by subtracting EXTRA_LOWER on Y
	local spawnCFrame =
	...
	-- ====== UNIFORM FADE-IN (store originals & tweens) ======
	...
	-- Collect all decals and textures for fading
	for _, d in ipairs(newDrop:GetDescendants()) do
		if d:IsA("Decal") or d:IsA("Texture") then
			d:SetAttribute("OrigT", d.Transparency)
			table.insert(fadeDecals, d)
			d.Transparency = 1
		end
	end
	...
	local function fadeOutAndDestroy()
		-- cancel any active fade-in to avoid weird stagger
		for _, tw in ipairs(activeTweens) do
			pcall(function() tw:Cancel() end)
		end
		-- unified fade to 1 for everything
		for _, bp in ipairs(fadeParts) do
			TweenService:Create(bp, TweenInfo.new(FADE_TIME, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
				{ Transparency = 1 }):Play()
		end
		for _, dd in ipairs(fadeDecals) do
			TweenService:Create(dd, TweenInfo.new(FADE_TIME, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
				{ Transparency = 1 }):Play()
		end

		-- disconnect listener and hard-destroy after fade
