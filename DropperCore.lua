--[[
	DropperCore v5.2 - MULTI-TYCOON SAFE with FIXED FADE OUT + BETTER COLLISION
	
	🔧 v5.2 FIXES:
	✅ FIXED: Added BeanieDrops2 to ALL_DROPPER_GROUPS
	✅ FIXED: Respects canCollide config parameter
	✅ FIXED: Better collision group setup logging
	
	🔧 v5.1 FIXES:
	✅ FIXED: Fade out works (stops physics, removes constraints, waits for tweens)
	✅ FIXED: No more 50-part limit on fade
	✅ FIXED: Proper timing before destroy
...
local ALL_DROPPER_GROUPS = {
	...
	-- Beanie groups ✅ ADDED!
	"BeanieDrops", "BeanieDrops1", "BeanieDrops2",
...
local function setupCollisionGroups(dropGroup: string, playerGroup: string)
	...
	print("🔧   ✅ Disabled cross-group collisions with", crossGroupCount, "other groups")
	print("🔧 [DropperCore] Collision setup complete!")
end
...
function Core.Run(config)
	...
	local CAN_COLLIDE = config.canCollide ~= false  -- ✅ Default true, but respect config!
	...
	while true do
		...
		part.CanQuery = true
		part.CanTouch = true
		part.CanCollide = CAN_COLLIDE  -- ✅ Respect config!
...
--============================
-- PUBLIC: Core.RunModel (clone + drop a Model) - FIXED FADE OUT!
--============================
function Core.RunModel(config)
	...
	local function spawnOne(isPrewarm)
		...
		local function collect()
			...
			-- ✅ CALCULATE PROPER FADE DURATION
