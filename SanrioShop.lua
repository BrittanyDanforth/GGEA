...
function Shop:createHomePage()
	...
	-- Add some featured gamepasses too
	for _, pass in ipairs(Core.DataManager.products.gamepasses) do
...
function Shop:selectTab(tabId)
	...
	if self.currentTab == tabId and self.tabs[tabId] then
		...
		Core.Animation.tween(self.tabs[tabId].button, {
			BackgroundColor3 = Core.Utils.blend(data.color, Color3.new(1, 1, 1), 0.9)
		}, Core.CONSTANTS.ANIM_FAST)

		local stroke = self.tabs[tabId].button:FindFirstChildOfClass("UIStroke")
		if stroke then
			stroke.Color = data.color
		end

		self.tabs[tabId].icon.ImageColor3 = data.color
		self.tabs[tabId].label.TextColor3 = data.color
...
MarketplaceService.PromptGamePassPurchaseFinished:Connect(function(player, passId, purchased)
	...
	if purchased then
		...
		shop:showNotification("Purchase Successful!", "You now own " .. pending.product.name, "success")
		Core.SoundSystem.play("success")
		...
		shop:showNotification("Purchase Cancelled", "Purchase was cancelled", "warning")
	end
end)
...
MarketplaceService.PromptProductPurchaseFinished:Connect(function(player, productId, purchased)
	...
	if purchased then
		shop:showNotification("Purchase Successful!", "You received " .. Core.Utils.formatNumber(pending.product.amount) .. " cash!", "success")
		Core.SoundSystem.play("success")
