--[[
	✨ HelloKitty Purchase Handler - ULTIMATE FIXED Version with AUTO-COLLECT + 2X CASH
	
	NEW FEATURES (Matching Kuromi Handler):
	...
	✅ Proper handling of "Drop_" model drops from HelloKittyPL template
	✅ FIXED: AUTO indicator now matches Kuromi's cyan appearance
--]]
...
local function setupAutoCollect(player)
	...
	-- Create auto-collect indicator - FIXED: NOW MATCHES KUROMI'S CYAN COLOR!
	if giver then
		...
		if autoCollectEnabled[player] ~= false then
			frame.BackgroundColor3 = Color3.new(0, 0.7, 0.7)  -- FIXED: Cyan to match Kuromi!
			frame.BackgroundTransparency = 0.2
			label.Text = "AUTO"
			label.TextColor3 = Color3.new(1, 1, 1)
		else
...
-- Auto-collect toggle handler - FIXED: NOW MATCHES KUROMI'S CYAN COLOR!
...
-- Setup button dependency system - NOW CHECKS FOR SPAWNED OBJECTS LIKE KUROMI
local function setupButtonDependency(button)
	...
	if dependency and dependency.Value and dependency.Value ~= "" then
		...
		local function checkDependency()
			-- CHECK FOR SPAWNED OBJECTS, NOT BUTTON NAMES!
...
local function resetTycoonPurchases()
	...
	print("  ✓ [HelloKitty] Destroyed", destroyedCount, "cash drops (models + parts)")
...
-- ========================================
-- INSTANT PART COLLECTOR FOR MODEL DROPS - UPDATED FOR HELLOKITTY!
-- ========================================
...
print("✅ [HelloKitty] Purchase Handler ULTIMATE FIXED loaded with AUTO-COLLECT + 2X CASH!")
...
print("⚡ [HelloKitty] INSTANT MODEL DROP COLLECTION for DropperCore compatibility")
...
print("🎨 [HelloKitty] AUTO indicator now matches Kuromi's cyan appearance!")
